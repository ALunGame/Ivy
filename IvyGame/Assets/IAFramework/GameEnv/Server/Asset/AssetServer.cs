...
namespace IAFramework.Server
{
    public class AssetServer : BaseServer
    {
        ...
        private string audioPackageName;
        private ResourcePackage audioPackage;
        public ResourcePackage AudioPackage
        {
            get
            {
                if (audioPackage == null)
                {
                    audioPackage = YooAssets.GetPackage(audioPackageName);
                }
                return audioPackage;
            }
        }
        ...
        /// </summary>
        /// <param name="pPackageName"></param>
        public void SetAudioPackage(string pPackageName)
        {
            audioPackageName = pPackageName;
            audioPackage = YooAssets.GetPackage(pPackageName);
            Debug.LogError($"SetAudioPackage::{audioPackage.PackageName}");
        }
        ...
        /// <returns></returns>
        public AudioClip LoadAudioClip(string location)
        {
            AssetHandle handle = audioPackage.LoadAssetSync<AudioClip>(location);
